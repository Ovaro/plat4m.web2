...
public class MnyObjectUtil {
    ...
    private static final Logger log = LoggerFactory.getLogger(MnyObjectUtil.class);
    ...
    public static Double calculateInvestmentBalance(Account account, Date date, MnyContext mnyContext) {
        ...
        Collections.sort(
            securityHoldings,
            new Comparator<SecurityHolding>() {
                @Override
        ...
        for (SecurityHolding sec : securityHoldings) {
            if (log.isDebugEnabled()) {
                log.debug(
                    "securityName=" +
                    sec.getSecurity().getName() +
                    ", quantity=" +
                    account.formatSecurityQuantity(sec.getQuantity()) +
                    ", price=" +
                    account.formatAmmount(sec.getPrice()) +
                    ", value=" +
                    account.formatAmmount(sec.getMarketValue())
                );
            }
