...
public class AccountImplUtil {
    ...
    public static Account getAcccount(Map<String, Object> row) {
        ...
        account.setSerialDate(serialDate);

        Date openedDate = (Date) row.get("dtOpen");
        account.setDateOpened(openedDate);

        // sguid
        String sguid = (String) row.get("sguid");
        account.setGuid(sguid);

        // sguid
        Integer fi = (Integer) row.get("hfi");
        account.setFI(fi);
