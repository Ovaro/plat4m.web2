...
public class GenBruteForce {
    ...
    private static final Logger log = LoggerFactory.getLogger(GenBruteForce.class);
    ...
    private static char[] createUSKeyboardMnyAlphabets() {
        return appendCharArrays(
            ALPHABET_UPPERS,
            ALPHABET_DIGITS,
            ALPHABET_SPECIAL_CHARS_1,
            ALPHABET_SPECIAL_CHARS_2,
            ALPHABET_SPECIAL_CHARS_3,
            ALPHABET_SPECIAL_CHARS_4
        );
    }
    ...
    private GenBruteForceContext createNextContext(GenBruteForceContext context) {
        ...
        GenBruteForceContext newContext = new GenBruteForceContext(
            context.getBuffer(),
            context.getBufferLen(),
            context.getMask(),
            newCursor,
            context.getAlphabets(),
        ...
        try {
            for (int index = startIndex; index < len; index++) {
                ...
                if (resumeContext != null) {
                    int n = PasswordUtils.compareCursorIndexes(
                        context.getCurrentCursorIndex(),
                        resumeContext.getCurrentCursorIndex(),
                        context.getAlphabetsLen()
                    );
                    if (n < 0) {
                        continue;
    ...
    private long handleKnownCharInMask(GenBruteForceContext context, char maskChar, long count) {
        ...
        if (terminate) {
            log.warn("Terminate early at cursor=" + context.getCursor() + ", alphabet=" + c + ", alphabetLen=" + context.getAlphabetsLen());
        } else {
            GenBruteForceContext newContext = createNextContext(context);
            long n = generateString(newContext);
            count += n;
