...
public class AccountUtil {
    ...
    public static List<Transaction> retrieveTransactions(
        ...
        final List<TransactionFilter> transactionFilters
    ) throws IOException {
            ...
            while (cursor.moveToNextRow()) {
                if (account != null) {
                    if (cursor.currentRowMatches(rowPattern)) {
                        ...
                        TransactionImplUtil.addTransactionFromRow(
                            db,
                            account.getId(),
                            transactionFilters,
                            row,
    ...
    private static List<TransactionFilter> getTransactionFilters() {
        ...
        // https://jythonpodcast.hostjava.net/jythonbook/chapter10.html
        // transactionFilters =
    ...
    public static void handleSplit(Database db, List<Transaction> transactions) throws IOException {
    ...
    public static void handleSplitNoRemove(Database db, List<Transaction> transactions) throws IOException {
        String splitTableName = "TRN_SPLIT";
        Table splitTable = db.getTable(splitTableName);
        Cursor cursor = Cursor.createIndexCursor(splitTable, splitTable.getIndex("htrnTrnSplit"));
        // Cursor cursor = Cursor.createCursor(splitTable);
    ...
    private static Account getAccount(Integer relatedToAccountId, MnyContext mnyContext) throws IOException {
        ...
        if (cursor.findRow(rowPattern)) { // JMK: was findFirstRow
            Map<String, Object> row = cursor.getCurrentRow();
